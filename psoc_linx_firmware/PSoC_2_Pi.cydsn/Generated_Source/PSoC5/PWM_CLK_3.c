...
void PWM_CLK_3_Start(void) 
{
    ...
    PWM_CLK_3_CLKEN |= PWM_CLK_3_CLKEN_MASK;
	PWM_CLK_3_CLKSTBY |= PWM_CLK_3_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: PWM_CLK_3_Stop
********************************************************************************
*
* Summary:
...
void PWM_CLK_3_Stop(void) 
{
    ...
    PWM_CLK_3_CLKEN &= (uint8)(~PWM_CLK_3_CLKEN_MASK);
	PWM_CLK_3_CLKSTBY &= (uint8)(~PWM_CLK_3_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: PWM_CLK_3_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((PWM_CLK_3_CLKEN & PWM_CLK_3_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = PWM_CLK_3_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = PWM_CLK_3_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* PWM_CLK_3__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(PWM_CLK_3_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(PWM_CLK_3_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: PWM_CLK_3_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: PWM_CLK_3_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = PWM_CLK_3_GetSourceRegister();
    uint16 oldDivider = PWM_CLK_3_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = PWM_CLK_3_CLKEN & PWM_CLK_3_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((PWM_CLK_3_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: PWM_CLK_3_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(PWM_CLK_3_DIV_PTR);
}


/*******************************************************************************
* Function Name: PWM_CLK_3_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    PWM_CLK_3_MOD_SRC |= modeBitMask & (uint8)PWM_CLK_3_MODE_MASK;
}


/*******************************************************************************
* Function Name: PWM_CLK_3_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    PWM_CLK_3_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(PWM_CLK_3_MODE_MASK));
}


/*******************************************************************************
* Function Name: PWM_CLK_3_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return PWM_CLK_3_MOD_SRC & (uint8)(PWM_CLK_3_MODE_MASK);
}


/*******************************************************************************
* Function Name: PWM_CLK_3_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = PWM_CLK_3_GetDividerRegister();
    uint8 oldSrc = PWM_CLK_3_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        PWM_CLK_3_MOD_SRC |= CYCLK_SSS;
        PWM_CLK_3_MOD_SRC =
            (PWM_CLK_3_MOD_SRC & (uint8)(~PWM_CLK_3_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: PWM_CLK_3_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return PWM_CLK_3_MOD_SRC & PWM_CLK_3_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: PWM_CLK_3_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    PWM_CLK_3_PHASE = clkPhase & PWM_CLK_3_PHASE_MASK;
}


/*******************************************************************************
* Function Name: PWM_CLK_3_GetPhase
********************************************************************************
*
* Summary:
...
{
    return PWM_CLK_3_PHASE & PWM_CLK_3_PHASE_MASK;
}
