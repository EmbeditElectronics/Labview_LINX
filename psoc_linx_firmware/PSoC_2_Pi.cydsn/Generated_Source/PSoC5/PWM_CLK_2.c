...
*
*******************************************************************************/
void PWM_CLK_2_Start(void) 
{
    /* Set the bit to enable the clock. */
    PWM_CLK_2_CLKEN |= PWM_CLK_2_CLKEN_MASK;
	PWM_CLK_2_CLKSTBY |= PWM_CLK_2_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: PWM_CLK_2_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void PWM_CLK_2_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    PWM_CLK_2_CLKEN &= (uint8)(~PWM_CLK_2_CLKEN_MASK);
	PWM_CLK_2_CLKSTBY &= (uint8)(~PWM_CLK_2_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: PWM_CLK_2_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void PWM_CLK_2_StopBlock(void) 
{
    if ((PWM_CLK_2_CLKEN & PWM_CLK_2_CLKEN_MASK) != 0u)
    {
...
#if defined(PWM_CLK_2__CFG3)
        CLK_DIST_AMASK = PWM_CLK_2_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = PWM_CLK_2_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* PWM_CLK_2__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(PWM_CLK_2_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(PWM_CLK_2_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: PWM_CLK_2_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void PWM_CLK_2_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        PWM_CLK_2_CLKSTBY &= (uint8)(~PWM_CLK_2_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: PWM_CLK_2_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void PWM_CLK_2_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = PWM_CLK_2_GetSourceRegister();
    uint16 oldDivider = PWM_CLK_2_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = PWM_CLK_2_CLKEN & PWM_CLK_2_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((PWM_CLK_2_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: PWM_CLK_2_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 PWM_CLK_2_GetDividerRegister(void) 
{
    return CY_GET_REG16(PWM_CLK_2_DIV_PTR);
}


/*******************************************************************************
* Function Name: PWM_CLK_2_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void PWM_CLK_2_SetModeRegister(uint8 modeBitMask) 
{
    PWM_CLK_2_MOD_SRC |= modeBitMask & (uint8)PWM_CLK_2_MODE_MASK;
}


/*******************************************************************************
* Function Name: PWM_CLK_2_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void PWM_CLK_2_ClearModeRegister(uint8 modeBitMask) 
{
    PWM_CLK_2_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(PWM_CLK_2_MODE_MASK));
}


/*******************************************************************************
* Function Name: PWM_CLK_2_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 PWM_CLK_2_GetModeRegister(void) 
{
    return PWM_CLK_2_MOD_SRC & (uint8)(PWM_CLK_2_MODE_MASK);
}


/*******************************************************************************
* Function Name: PWM_CLK_2_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void PWM_CLK_2_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = PWM_CLK_2_GetDividerRegister();
    uint8 oldSrc = PWM_CLK_2_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        PWM_CLK_2_MOD_SRC |= CYCLK_SSS;
        PWM_CLK_2_MOD_SRC =
            (PWM_CLK_2_MOD_SRC & (uint8)(~PWM_CLK_2_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: PWM_CLK_2_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 PWM_CLK_2_GetSourceRegister(void) 
{
    return PWM_CLK_2_MOD_SRC & PWM_CLK_2_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: PWM_CLK_2_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void PWM_CLK_2_SetPhaseRegister(uint8 clkPhase) 
{
    PWM_CLK_2_PHASE = clkPhase & PWM_CLK_2_PHASE_MASK;
}


/*******************************************************************************
* Function Name: PWM_CLK_2_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 PWM_CLK_2_GetPhaseRegister(void) 
{
    return PWM_CLK_2_PHASE & PWM_CLK_2_PHASE_MASK;
}
